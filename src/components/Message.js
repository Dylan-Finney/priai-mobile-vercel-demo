...
export const Message = ({key,  time, username, aiName, speaker, message, calloutAgent, keepMessageAsIs, lastMessage, error, loading}) => {
    ...
    return (
        <Flex
            ...
            <Flex flexDir={"row"}>
            <Box>
            {speaker === username ? <UserAvatar /> :  agentsImages[speaker]?.chatIcon ? agentsImages[speaker]?.chatIcon :  <AIAvatar/>}
            </Box>
            <Flex
            width={"100%"}
            style={{
            ...
            height={"fit-content"}
                ...
                {speaker === username ? `You (${username})` : speaker === "Personal Assistant" ? (aiName !== "" ? aiName : `${username}'s personal AI assistant`) : speaker}
            </Text>
            <Spacer/>
            {
            speaker === username ? (
            ...
            <Text fontWeight={400} fontSize={"14px"} color={"#F04438"}>{error.status === 503 ? (
